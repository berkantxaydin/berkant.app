...
Write-Output "`n--- 🚀 Starting Platform Recovery & Startup ---"
...
Write-Output "------------------------------------------------"
if (Get-Process nginx -ErrorAction SilentlyContinue) { Write-Output "✅ Nginx: [RUNNING]" } else { Write-Output "❌ Nginx: [FAILED]" }

try {
    $response = Invoke-WebRequest -Uri "http://127.0.0.1:5000/health" -TimeoutSec 5 -UseBasicParsing
    if ($response.StatusCode -eq 200) { Write-Output "✅ App (Waitress): [ACTIVE] (API OK)" }
} catch {
    Write-Output "❌ App (Waitress): [UNREACHABLE] - API did not respond"
}

$cf = Get-Service cloudflared -ErrorAction SilentlyContinue
if ($cf -and $cf.Status -eq "Running") { Write-Output "✅ Tunnel: [ACTIVE]" }
