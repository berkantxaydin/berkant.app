...
class AnalyticsRepository(BaseRepository):
    ...
    def log_request(self, method, path, visitor_id, is_htmx, status_code, duration_ms, ip_address=None):
        query = """
            INSERT INTO Analytics_Logs (method, path, ip_address, visitor_id, is_htmx, status_code, duration_ms)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """
        self.execute(query, (method, path, ip_address, visitor_id, 1 if is_htmx else 0, status_code, duration_ms), commit=True)
