...
class JamRepository(BaseRepository):
    ...
    def create_jam(self, title: str, theme: str, start_time: str, end_time: str, youtube_url: Optional[str] = None) -> int:
    ...
    def update_jam(self, jam_id: int, title: str, theme: str, start_time: str, end_time: str, youtube_url: Optional[str] = None) -> bool:
        query = """
            UPDATE Game_Jams 
            SET title = ?, theme = ?, start_time = ?, end_time = ?, youtube_url = ? 
            WHERE id = ?
        """
        self.execute(query, (title, theme, start_time, end_time, youtube_url, jam_id), commit=True)
        return True
